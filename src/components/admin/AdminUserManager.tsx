    ...
    <section className="panel mt-6 rounded-3xl p-4 sm:p-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex w-full flex-col gap-2 sm:flex-row lg:max-w-2xl">
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search users" className="w-full rounded-2xl border border-white/15 bg-black/40 px-4 py-3 text-sm text-white outline-none transition focus:border-rose-300/60" />
            ...
            <article key={user.id} className="rounded-2xl border border-white/10 bg-black/35 p-4">
              <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-300 line-clamp-1 md:line-clamp-none">{user.email} · {user.region} · {user.preferredPlatform ?? 'No platform set'}</p>
                  <p className="mt-2 text-xs uppercase tracking-[0.12em] text-slate-500 line-clamp-2 md:line-clamp-none">SR {user.skillRating} · Honour {user.honourScore} · Entries {user._count.raceRegistrations} · Organised {user._count.raceSlotsOrganised} · Disputes {user._count.disputesOpened}</p>
                </div>

                <div className="mobile-scroll-row sm:grid sm:gap-2 sm:overflow-visible sm:grid-cols-4">
                  <button disabled={busy} onClick={() => updateUser(user.id, { role: 'PLAYER' })} className="mobile-scroll-card min-h-11 rounded-xl border border-white/20 bg-white/5 px-3 py-2 text-xs text-slate-100 transition hover:bg-white/10 disabled:opacity-45 sm:min-w-0">Set PLAYER</button>
              ...
              <div className="mt-4 rounded-2xl border border-white/10 bg-white/[0.03] p-4">
                ...
                <div className="mt-3 grid gap-3 md:grid-cols-2 lg:grid-cols-[120px_120px_1fr_auto]">
                  ...
                  <input value={form.reason} onChange={(event) => setRatingForm(user.id, { reason: event.target.value })} placeholder="Reason / steward note" className="rounded-xl border border-white/15 bg-black/40 px-3 py-2 text-sm text-white outline-none focus:border-cyan-300/60" />
                  <button disabled={busy} onClick={() => saveRatings(user)} className="min-h-11 rounded-xl border border-cyan-300/40 bg-cyan-500/15 px-4 py-2 text-xs uppercase tracking-[0.14em] text-cyan-100 transition hover:bg-cyan-500/25 disabled:opacity-45">Save ratings</button>
                </div>
